-- 建表 DDL
-- 挂载于 MySQL 容器的 /docker-entrypoint-initdb.d，只在数据卷为空时执行一次；
-- 改过本文件要 docker compose down -v 清卷重建才会重跑。
-- 库名 trade_reconcile，InnoDB，utf8mb4；表名统一 t_ 前缀（ORDER 是保留字）；
-- 金额一律 DECIMAL(18,2)；CHECK 约束只是兜底，防超卖靠条件 UPDATE 的 WHERE。
-- 标注 V1.5 的字段只留列结构，不实现对应功能。
...
-- 驱动兼容：Drogon 的 MySQL 客户端（容器内走 MariaDB 客户端库）没开
-- 服务器公钥/TLS，连不上 caching_sha2_password 的完整认证，表现为事务
-- 等连接超时。开发环境统一用 mysql_native_password（MySQL 8 仍支持）。
-- 前提：docker-compose.yml 已设 MYSQL_ROOT_HOST="%"。
...
-- 只增不改（无 updated_at），是日终对账的本地数据源。
-- uk_request_biz 防同一请求重复记账。
...
-- t_product 商品表
-- V1 直接扣减/归还 stock，不引入锁定库存（locked_stock 属 V1.5）。
...
-- t_order 订单表（ORDER 是保留字，不能用裸表名）
-- uk_request_id 是幂等兜底。
-- 状态机单向禁止回退：V1 实现 0→1→2 与 0→3（超时关单归还库存）；1→4 退款属 V1.5。
...
-- callback_time 是对账口径（按渠道回调时间归集），与 created_at 分离。
...
-- t_idempotent 幂等记录表
-- 幂等记录与业务写入同一事务；status=0 冲突返回 10003 稍后重试；
-- 先到者失败后，后到者重试会重走整个流程（业务自身幂等兜底）。
...
-- 与业务写操作同事务 INSERT，由 MessageRelayTask 异步投递 MQ。

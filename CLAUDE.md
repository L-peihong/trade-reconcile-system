...
5. **对账链路** ✅ —— `gen_bill.sh` + 日终任务 + 差异查询/核销 ← 亮点④（验收：注入三类差异各 1 笔 → 跑批 diffCount=3、类型/金额差分毫不差；核销 → handleStatus=1；重复核销 70003；同日重跑 70002 幂等）
6. **收尾** ✅ —— 单测 27 用例 + `smoke_test.sh`(8 步端到端全绿)+ `docs/design.md`(面试问答大纲)+ README + docs/api.md

...
void NotifyConsumer::handleMessage(const amqp_envelope_t& envelope) {
    ...
    // 成功才 ACK；失败 nack(requeue=false) 转死信。
    // ACK 前连接断开由 broker 重投，消费幂等兜底。
    if (process(body)) {
...
bool NotifyConsumer::process(const std::string& body) {
    ...
    for (;;) {
        ...
        req->addHeader("X-Request-Id", notifyNo);  // 雪花 ID 是十六进制字符集，能过过滤器校验

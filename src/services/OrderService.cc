...
// expire_time 应用侧算好随 INSERT 写入，不回读 DB。
// 时区显式 UTC+8（北京时间），不依赖进程 TZ：容器里 TZ 环境变量不一定生效，
// 依赖 localtime 会与 MySQL 的 +8:00 差 8 小时。中国无夏令时，固定偏移精确。
...
OrderService::OrderService()
    ...
    // 构造里不能碰 drogon::app()：本对象可能随 Controller 静态注册被构造，
    // 那时 app 还没初始化。DB 客户端在运行时获取。
}
...
CreateOrderOutcome OrderService::createOrder(const models::CreateOrderReq& req) {
    ...
    try {
        // 事务 RAII：异常路径退出自动回滚。
        // 本方法跑在工作线程池线程，同步 DB API 不能在事件循环线程调用。
        utils::TransactionGuard txGuard(utils::beginTransaction());
        auto tx = txGuard.tx();

        // 1. 幂等抢占，与业务同事务：后面任何失败回滚，幂等键一并释放。
        const auto outcome =
        ...
        // 5. 幂等快照写回（同事务：业务成功才留痕）。
        //    快照要和首次在线响应逐字节一致：drogon 的 newHttpJsonResponse
        //    用 StreamWriterBuilder 紧凑序列化，这里用相同配置。
        //    注意不能用 toStyledString，那是美化格式，重放字节会不一样。
        const auto response =
        ...
        // 技术细节进日志，message 只给通用话术。
        // 注意：DrogonDbException 没有 what()，用 base().what()。
